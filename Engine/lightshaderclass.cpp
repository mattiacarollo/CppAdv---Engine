...
bool LightShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	result = ShaderLoader::vertexPreBuiltLoad(L"./lightVS.cso", device, &m_vertexShader, &vs);
	if (FAILED(result))
	{
	...
	result = ShaderLoader::pixelPreBuiltLoad(L"./lightPS.cso", device, &m_pixelShader, &ps);
	if (FAILED(result))
	{
		// If the shader failed to compile it should have writen something to the error message.
		if (errorMessage)
	...
	result = ShaderLoader::createInputLayout(L"./lightVS.cso", device, polygonLayout, numElements, &m_layout);
	if (FAILED(result))
	{
		return false;
	}
